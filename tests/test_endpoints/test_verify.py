...
class TestVerify(unittest.TestCase):
    ...
    def test_conforms_to_schema(self):
        """Response conforms to the VerifyResult OpenAPI schema."""
        check_conforms_to_openapi_schema(self, TEST_URL, "VerifyResult")
    ...
    def test_threshold_param_changes_results(self):
        ...
        header = fetch_header(self.client, role=ROLE_OWNER)
        rv_strict = self.client.get(TEST_URL + "?oldage=50", headers=header)
        rv_lenient = self.client.get(TEST_URL + "?oldage=200", headers=header)
        self.assertEqual(rv_strict.status_code, 200)
        self.assertEqual(rv_lenient.status_code, 200)
    ...
    def test_invalid_param_rejected(self):
        ...
        header = fetch_header(self.client, role=ROLE_OWNER)
        rv = self.client.get(TEST_URL + "?oldage=notanumber", headers=header)
