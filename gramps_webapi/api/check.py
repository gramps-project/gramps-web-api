...
from gramps.gen.lib.serialize import object_to_dict, data_to_object
from gramps.plugins.tool.check import CheckIntegrity

from .util import strip_whitespace_recursive
...
def check_database(db_handle: DbWriteBase, progress_cb: Optional[Callable] = None):
    ...
    if whitespace_fixes > 0:
        text += f"\n\nStripped trailing whitespace from {whitespace_fixes} objects."
    
    return {"num_errors": errs, "message": text}


def strip_trailing_whitespace(db_handle: DbWriteBase, progress_cb: Optional[Callable] = None) -> int:
    ...
    object_types = [
        ("Person", Person, db_handle.get_person_handles, db_handle.get_person_from_handle, db_handle.commit_person),
        ("Family", Family, db_handle.get_family_handles, db_handle.get_family_from_handle, db_handle.commit_family),
        ("Event", Event, db_handle.get_event_handles, db_handle.get_event_from_handle, db_handle.commit_event),
        ("Place", Place, db_handle.get_place_handles, db_handle.get_place_from_handle, db_handle.commit_place),
        ("Source", Source, db_handle.get_source_handles, db_handle.get_source_from_handle, db_handle.commit_source),
        ("Citation", Citation, db_handle.get_citation_handles, db_handle.get_citation_from_handle, db_handle.commit_citation),
        ("Repository", Repository, db_handle.get_repository_handles, db_handle.get_repository_from_handle, db_handle.commit_repository),
        ("Media", Media, db_handle.get_media_handles, db_handle.get_media_from_handle, db_handle.commit_media),
        ("Note", Note, db_handle.get_note_handles, db_handle.get_note_from_handle, db_handle.commit_note),
        ("Tag", Tag, db_handle.get_tag_handles, db_handle.get_tag_from_handle, db_handle.commit_tag),
    ]
    
    with DbTxn("Strip trailing whitespace", db_handle, batch=True) as trans:
        for obj_type_name, obj_class, get_handles, get_obj, commit_obj in object_types:
            for handle in get_handles():
                try:
                    ...
                    print(f"Error processing {obj_type_name} {handle}: {e}")
                    continue
